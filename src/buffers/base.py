...
class ReplayBufferBase(ABC):
    def __init__(self, size: int, scans_dim: int, vehicle_info_dim: int, action_dim: int):
        ...
        self.buffer = {
            ...
            "vehicle_info": np.zeros((size, vehicle_info_dim), dtype=np.float32),
            ...
            "next_vehicle_info": np.zeros((size, vehicle_info_dim), dtype=np.float32),
    ...
    @abstractmethod
    def add(self, scans: torch.Tensor, vehicle_info: torch.Tensor, action: torch.Tensor, reward: float, next_scans: torch.Tensor, next_vehicle_info: torch.Tensor, done: bool):
        pass
